...
exports.show_possible_match = function(req, res) {
   if(req.session.loggedin == true && req.session.email) { // if user is logged in, fetch ID from log in in mySql
	   		...
	   		switch(what_to_do) {
	   			case 'match':
					...
					//Check if Match
					   		...
					   		checkMatch(function(match) { //insert in mySql match table
					   			if(match == 'no-match') {
